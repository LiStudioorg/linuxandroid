...
1. 点击发行版按钮 → 下载（可中断，再次点击**断点续传**，主源失败自动换镜像）
2. 校验 SHA256 → 解压 → 按钮变“启动 …”
3. 点击“启动” → 底部终端出输出；输入命令（如 `ls`、`cat /etc/os-release`）点“发送”或键盘发送键
4. rootfs 位于 `filesDir/rootfs/<id>`，重装＝清除应用数据
